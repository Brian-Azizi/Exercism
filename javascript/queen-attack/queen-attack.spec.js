...
describe("Queens", () => {
  describe.each([["white"], ["black"]])(
    "Test creation of %s Queen with valid and invalid positions",
    (colour) => {
      test("queen with a valid position", () => {
        const queens = new QueenAttack({ [colour]: [2, 2] });
        expect(queens[colour]).toEqual([2, 2]);
      });

      test("queen must have positive row", () => {
        const positioning = { [colour]: [-2, 2] };
